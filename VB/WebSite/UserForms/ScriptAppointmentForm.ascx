...
<%@ Register Assembly="DevExpress.Web.ASPxScheduler.v9.2, Version=9.2.2.0, Culture=neutral, PublicKeyToken=b88d1754d700e49a"
	...
	ASPxClientAppointmentForm = _aspxCreateClass(ASPxClientFormBase, {
		...
		Update: function(apt) {
			...
			for(var i = 0; i<apt.resources.length; i++) {
				...
				if (resource != "null")
					_aspxArrayPush(values, resource);
			}
		...
		UpdateDdResourceControl: function(selectedItems) {
			...
			if (count > 0) {
				for(var i=0; i<count; i++)
					_aspxArrayPush(resourceNames, selectedItems[i].text);
			}
			else
				_aspxArrayPush(resourceNames, this.controls.ddResource.cp_Caption_ResourceNone);
